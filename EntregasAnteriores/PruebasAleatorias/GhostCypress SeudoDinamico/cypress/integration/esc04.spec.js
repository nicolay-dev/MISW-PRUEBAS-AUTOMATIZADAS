...
const titulo02 = faker.random.alphaNumeric(255);
const titulo03 = faker.random.alphaNumeric(255);
...
describe('Create and edit post', () => {
  ...
  it('Login to ghost, create title, create content, publish and edit title.2', () => {
    cy.get('form').within(() => {
      POM.signIn(username, password);
    })
    ...
    POM.elements.getPostPageinSite(titulo02).click()
    cy.wait(3000)
    
  })

  it('Login to ghost, create title, create content, publish and edit title.3', () => {
    cy.get('form').within(() => {
      POM.signIn(username, password);
    })
    ...
    //Cancel update post
    cy.get('.gh-publishmenu-footer > .gh-btn-outline > span').click();
    cy.wait(2000);
    cy.get('.gh-alert-close').click()
    POM.editTitlePost(titulo);
    cy.wait(2000);
    ...
    POM.elements.getPostPageinSite(titulo03).click()
    cy.wait(3000)
