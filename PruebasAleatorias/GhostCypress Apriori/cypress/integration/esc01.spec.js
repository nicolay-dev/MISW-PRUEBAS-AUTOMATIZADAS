...
describe('Change password', () => {
  ...
  // it('01-Login to ghost, change password and logout oráculo positivo ', () => {
  //   cy.get('form').within(() => {
...
//      //Se coloca los datos que si permitan continuar la prueba hasta el final
  ...
  it('01_4-Login to ghost, change password and logout oráculo negativo password old bien, passwordnewconfirm lleno con 9 caracteres', function() {
    cy.get('form').within(() => {
      POM.signIn(username, password);
    })
    cy.wait(1000)
    ...
    cy.get('#user-password-old').type(password)
    // Se coloca una contraseña igual a limite menos 1 
    cy.get('#user-password-new').type(this.data.apriori[1].password9)
    cy.get('#user-new-password-verification').type(this.data.apriori[1].password9)
    cy.get('button span:contains("Change Password")').click()
    // Assert de que la app no debe dejar pasar campos vacíos 
    cy.get('button span:contains("Retry")').should('be.visible')
